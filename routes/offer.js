	...
	try {
		...
		if (user) {
			...
			if (req.files.files) {
				...
				const cloudImg = await cloudinary.v2.uploader.upload(picture, (error, result) => {
					if (error) {
...
router.get('/offer/:id', async (req, res) => {
	try {
		...
		const selectedOffer = await Offer.findOne({ _id: id }).populate({
			path: 'creator',
			model: User,
			select: { _id: 1, 'account.phone': 1, 'account.username': 1 }
		});
		console.log('findOne offer', selectedOffer);
